    ...
    <form action="{{ route('admin.deals.update', $deal) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="car_id" id="car_id" class="form-select @error('car_id') is-invalid @enderror" required>
                ...
                @foreach($cars as $car)
                    <option value="{{ $car->id }}" {{ (old('car_id', $deal->car_id) == $car->id) ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            ...
            <select name="product_id" id="product_id" class="form-select @error('product_id') is-invalid @enderror">
                ...
                @foreach($products as $product)
                    <option value="{{ $product->id }}" {{ (old('product_id', $deal->product_id) == $product->id) ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            ...
            <select name="employee_id" id="employee_id" class="form-select @error('employee_id') is-invalid @enderror" required>
                ...
                @foreach($employees as $employee)
                    <option value="{{ $employee->id }}" {{ (old('employee_id', $deal->employee_id) == $employee->id) ? 'selected' : '' }}>
                        {{ $employee->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="deal_date" id="deal_date" class="form-control @error('deal_date') is-invalid @enderror" value="{{ old('deal_date', $deal->deal_date->format('Y-m-d')) }}" required>
            @error('deal_date')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" name="amount" id="amount" class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $deal->amount) }}" required>
            @error('amount')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
